	...
	if (event_base == WIFI_EVENT)
	{
		switch (event_id)
		{
			case WIFI_EVENT_AP_START:
				...
				status_led_set_state(LED_BLINK_FAST);   // fast blink while trying to connect
				break;
			...
			case WIFI_EVENT_STA_CONNECTED:
				ESP_LOGI(TAG, "WIFI_EVENT_STA_CONNECTED");
				 status_led_set_state(LED_ON);  // solid ON when connected
				break;

			case WIFI_EVENT_STA_DISCONNECTED:
				ESP_LOGI(TAG, "WIFI_EVENT_STA_DISCONNECTED");
				 status_led_set_state(LED_BLINK_SLOW);   // slow blink when disconnected
				break;
		}
	}
...
void wifi_app_start(void)
{
	...
	status_led_init();
    status_led_set_state(LED_BLINK_SLOW);  // initial: slow blink while idle
